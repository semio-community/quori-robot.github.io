...
The sync script filters entries by the `sites` field in each MDX file — only entries tagged `quori` are copied here. Asset paths that don't exist locally are stripped.
