...
| **this repo** | Quori robot platform website (Astro + React) |
...
3. Set `visibility` in the frontmatter to include `"quori-robot"` if needed
...
- `hardware` — robotics hardware projects (includes Quori platform)
...
- **`src/pages/platform.astro`** — Platform overview page with module configurator
- **`src/components/ui/Configurator.tsx`** — Module selection UI with rules engine
- **`src/components/ui/Configurator3D.tsx`** — 3D hardware configurator (Three.js/WebGL)
- **`src/components/ui/ModuleCarousel.tsx`** — 3D carousel for hardware module configs
- **`src/components/ui/configurator3d/`** — Full 3D visualization system

These are tightly coupled to Quori hardware and should remain site-specific.
...
- Detail page components (`BaseDetailLayout`, `DetailHero`, `InfoCard`, etc.)
