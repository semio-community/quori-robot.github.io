...
export function ModuleCarousel({
	...
	useEffect(() => {
		...
		// biome-ignore lint/style/noNonNullAssertion: array is guaranteed non-empty by the length check above
	...
	const goToIndex = useCallback(
		(index: number) => {
			const nextId = orderedConfigurationIds[wrapIndex(index, orderedConfigurationIds.length)];
			if (!nextId) return;
	...
	}, [autoAdvance, autoAdvanceMs, controlsHovered, goNext, orderedConfigurationIds.length]);
	...
	return (
		<div className={clsx("w-full dark:bg-special-lighter/60 bg-neutral-200/60", className)}>
