		...
		<div className={clsx("w-full space-y-4", className)}>
			{title ? (
				<div className="text-center space-y-1">
					...
					{caption ? <p className="text-sm text-color-600 dark:text-color-400">{caption}</p> : null}
				</div>
			) : null}
			...
			<div className="hidden md:block overflow-hidden rounded-xl border border-border-subtle shadow-sm">
				<table className="w-full border-collapse">
					<thead className="bg-special-lighter">
						<tr>
							<th className="text-left px-4 py-3 text-sm font-semibold text-foreground">Feature</th>
							{columns.map((col) => (
								<th
									key={col.key}
									className={clsx(
					...
					<tbody>
						{rows.map((row, idx) => (
							<tr key={row.label} className={idx % 2 === 0 ? "bg-surface" : "bg-surface/70"}>
				...
				{rows.map((row) => (
					...
					>
						<div className="mb-3">
							...
							{row.description ? (
								<p className="text-xs text-color-600 dark:text-color-400">{row.description}</p>
							) : null}
						</div>
						<div className="grid gap-2">
							{columns.map((col) => (
								<div key={`${row.label}-${col.key}-mobile`} className="flex items-start gap-2">
									<span
										className={clsx(
