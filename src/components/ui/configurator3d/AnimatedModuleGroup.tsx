...
export function AnimatedModuleGroup({
	...
	const ensureMaterialsInitialized = useCallback(() => {
		...
		group.traverse((obj) => {
			...
			if (Array.isArray(currentMaterial)) {
				...
				for (const material of clones) collected.add(material);
			} else {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: ensureMaterialsInitialized and invalidate are stable callbacks; including them would cause infinite re-renders
	useEffect(() => {
		const group = groupRef.current;
		if (!group) return;
		...
		// biome-ignore lint/suspicious/noAssignInExpressions: intentional pattern to atomically increment and capture token
		...
		if (entering || exiting) {
			...
			controlsRef.current.opacity = animate(opacityValueRef.current, targetOpacity, {
				duration: 0.28,
				ease: "easeOut",
		...
		<group ref={groupRef} position={initialPositionRef.current}>
			{renderModel ? (
				<React.Suspense fallback={null}>
					<ModelMountEffect onMount={ensureMaterialsInitialized}>{module.model}</ModelMountEffect>
				</React.Suspense>
			) : null}
		</group>
	);
