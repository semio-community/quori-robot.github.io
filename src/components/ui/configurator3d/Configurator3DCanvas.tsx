...
import { CanvasVisibilityContext, useCanvasVisibility } from "./CanvasVisibilityContext";
import { StaticGroundShadow } from "./StaticGroundShadow";
import type { ConfigurationSpecification, ModuleId, ModuleSpecification, Vec3 } from "./types";
...
function DemandPresentationControls({
	...
	useFrame((state, delta) => {
		...
		const needsUpdate =
			...
			Math.abs(ref.current.rotation.x - (animation.rotation?.[0] ?? 0)) > 0.0005 ||
			Math.abs(ref.current.rotation.y - (animation.rotation?.[1] ?? 0)) > 0.0005 ||
			Math.abs(ref.current.rotation.z - (animation.rotation?.[2] ?? 0)) > 0.0005;
	...
	const bind = useGesture(
		{
			...
			onDrag: ({ down, delta: [x, y], memo: [oldY, oldX] = animation.rotation }) => {
						...
						>
							<group position={worldOffset} rotation={[0.1, -0.4, 0]}>
								{Object.entries(modules).map(([key, module]) => {
									const position = activeConfiguration?.modulePositions?.[module.id] ?? undefined;
...
export const Configurator3DCanvas = memo(Configurator3DCanvasImpl, areCanvasPropsEqual);
