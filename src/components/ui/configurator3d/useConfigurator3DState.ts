...
export function useConfigurator3DState({
	...
	const defaultConfigurationId = useMemo(() => {
		...
		// biome-ignore lint/style/noNonNullAssertion: configurationEntries.length === 0 is checked above, so [0] is guaranteed non-null
	...
	const [enterFromByModule, setEnterFromByModule] = useState<Record<ModuleId, Vec3>>({});
	const [exitViaByModule, setExitViaByModule] = useState<Record<ModuleId, Vec3>>({});
	...
	const setConfigurationId = useCallback(
		(nextId: ConfigurationId) => {
			...
			for (const module of moduleList) {
				const prevTarget = prevConfig.modulePositions?.[module.id] as Vec3 | undefined;
				const nextTarget = nextConfig.modulePositions?.[module.id] as Vec3 | undefined;
	...
	useEffect(() => {
		...
		const defaultChanged = lastDefaultConfigurationIdRef.current !== defaultConfigurationId;
	...
	}, [activeConfigurationId, configurations, defaultConfigurationId, setConfigurationId]);
	...
	const activeModuleSet = useMemo(() => {
		return setFromArray(Object.keys(activeConfiguration?.modulePositions ?? {}));
	}, [activeConfiguration]);
