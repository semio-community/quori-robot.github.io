...
import { areSetsEqual, isSubset, setDifferenceSize, setFromArray } from "./sets";
import type {
	ConfigurationEntry,
	ConfigurationId,
...
export function Configurator3DToggleControls({
	...
	const findBestToggleMatch = (moduleId: ModuleId, desired: Set<ModuleId>, turningOn: boolean) => {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: getNextConfigurationId is an inline function defined above; it captures its own deps and is intentionally excluded
	...
	}, [activeConfigurationId, activeModuleSet, canHover, configurationIndex, hoveredModuleId]);
			...
			>
				<div className="flex items-center justify-between gap-2 px-3 py-2">
					<div
						className={clsx(
							"min-w-0",
							desktopExpanded ? "opacity-100" : "hidden opacity-0 pointer-events-none",
						)}
					>
				...
				<ScrollArea.Root className="flex-1 min-h-0">
					...
					<ScrollArea.Scrollbar orientation="vertical" className="flex select-none touch-none p-1">
						<ScrollArea.Thumb className="relative flex-1 rounded-full bg-border-subtle" />
					</ScrollArea.Scrollbar>
				</ScrollArea.Root>
			</div>
