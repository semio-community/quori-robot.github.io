...
const moduleCards: ModuleCardDefinition[] = [
	...
	},
	{
		id: "speaker",
		name: "Smart Speaker",
		description: "Audio input/output module for spoken dialogue and human-robot interaction.",
		Icon: SpeakerIcon,
		accentClassName: "text-accent-base",
		imageSrc: {
	...
	},
	{
		...
		bullets: ["Sits on the front of the torso", "Customizable design for branding"],
	},
	{
		id: "arms",
		name: "Arms",
	...
	},
	{
		id: "torso",
		name: "Torso",
		description: "Adds stability and mounting space for arms and chest modules.",
		Icon: TorsoIcon,
		accentClassName: "text-accent-base",
		imageSrc: {
						...
						<div className="mb-4 sm:mb-0 sm:w-1/2">
							<div className="aspect-square sm:aspect-auto sm:h-full overflow-hidden bg-[#C9C9C9] dark:bg-[#4D4D4D] relative">
								{m.imageSrc ? (
									(() => {
										const source =
											typeof m.imageSrc === "string" ? { light: m.imageSrc } : m.imageSrc;
										return (
											<>
												<img
													src={source.light}
						...
						<div className="flex flex-col flex-1 p-6 pt-0 sm:pt-6 min-h-0">
							...
							{m.bullets.length > 0 ? (
								<div className="text-sm text-color-600 dark:text-color-400 mb-3 min-h-10">
									{m.bullets.map((item) => (
										<div key={`${m.id}-${item}`} className="flex items-start gap-2">
