...
export const configurator3DInitialConfigurationId = "base-torso-chest-arms-head-speaker";
...
const baseConfigurations: Record<string, ConfigurationSpecification> = {
	...
	"base-torso-chest-arms": {
		id: "base-torso-chest-arms",
		name: "Base + Torso + Chest + Arms",
		modulePositions: {
			base: [0, -0.75, 0],
			torso: [0, 0.9, 0],
			arms: [0, 1.15, -0.25],
...
function withStandSwap(cfg: ConfigurationSpecification): ConfigurationSpecification | null {
	...
	const modulePositions = { ...cfg.modulePositions } as Record<string, [number, number, number]>;
	// biome-ignore lint/performance/noDelete: deleting key from Record is the correct way to remove it; assignment to undefined would violate the type
...
const standConfigurations = Object.fromEntries(
	Object.values(baseConfigurations)
		...
		.filter(Boolean)
		// biome-ignore lint/style/noNonNullAssertion: cfg is guaranteed non-null after .filter(Boolean) above
		.map((cfg) => [cfg?.id, cfg!]),
);

export const configurator3DConfigurations: Record<string, ConfigurationSpecification> = {
	...baseConfigurations,
	...standConfigurations,
};
