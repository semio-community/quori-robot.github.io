...
export default function ContributorsSection({
	...
	const categoryLabel = partner?.category ? categoryLabels[partner.category] : undefined;
	...
	return (
		...
		>
			<div className="text-center text-color-600 dark:text-color-400 mb-6">
				...
				<br className="hidden md:block" /> spanning academia, industry, and government.
			</div>
			<div className="max-w-5xl mx-auto space-y-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<CallToActionButton href={url("/contributors")} size="large" variant="tertiary">
						See Contributors
					</CallToActionButton>
					...
					<div className="group bg-special-lighter rounded-lg hover:shadow-lg transition-all hover:scale-[1.01] h-full overflow-hidden backdrop-blur-lg flex flex-col md:flex-row md:items-stretch">
						...
						<div className="flex flex-col flex-1 p-6 pt-6">
							<div className="flex flex-wrap gap-2 mb-4">
								{categoryLabel && <BasicChip text="Sponsor" variant="secondary" />}
								{typeLabel && <BasicChip text={typeLabel} variant="default" />}
							</div>

							{displayName && (
								<h3 className="text-xl font-semibold text-accent-base mb-3">{displayName}</h3>
							)}
