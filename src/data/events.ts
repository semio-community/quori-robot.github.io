...
export async function filterEvents(criteria: {
	...
	if (criteria.dateFrom) {
		// biome-ignore lint/style/noNonNullAssertion: dateFrom is guaranteed non-null by the enclosing if-check
		events = events.filter((event) => event.data.startDate >= criteria.dateFrom!);
	}

	if (criteria.dateTo) {
		// biome-ignore lint/style/noNonNullAssertion: dateTo is guaranteed non-null by the enclosing if-check
...
export async function getEventsByOrganization(
	...
	return events.filter((event) =>
		(event.data.organizations ?? []).some((org) => org.organizationId.toLowerCase() === lower),
	);
}
