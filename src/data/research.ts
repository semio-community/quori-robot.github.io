...
export async function filterResearch(criteria: {
	...
	if (criteria.yearFrom !== undefined) {
		entries = entries.filter((entry) => {
			const year = getPublishYear(entry);
			// biome-ignore lint/style/noNonNullAssertion: yearFrom is guaranteed non-null by the enclosing if-check
	...
	if (criteria.yearTo !== undefined) {
		entries = entries.filter((entry) => {
			const year = getPublishYear(entry);
			// biome-ignore lint/style/noNonNullAssertion: yearTo is guaranteed non-null by the enclosing if-check
