...
const isVisible = <
	T extends {
		data: {
			draft?: boolean;
			sites?: readonly ("semio-community" | "quori" | "vizij")[];
		};
	},
>(
	entry: T,
...
const hasCustomOgImage = (entry: { data: Record<string, unknown> }) =>
	// biome-ignore lint/suspicious/noExplicitAny: ogImage is not typed on all collection entry types
...
async function loadOgImageAsset(
	...
	if (typeof meta === "string") {
		...
		if (originalFile) {
			...
			if (candidates?.length && candidates.length > 0) {
				...
				// biome-ignore lint/style/noNonNullAssertion: candidate is guaranteed non-null after the find above
				...
				// biome-ignore lint/style/noNonNullAssertion: cached is assigned above before this line
				return { src: cached! };
			}
		}
		return undefined;
	...
	// biome-ignore lint/style/noNonNullAssertion: dataUrl is assigned above before this line
